...
// import { AuthCard } from '@/components/AuthCard' // Disabled for testing
...
export default function Home() {
  const { user } = useAuthSession() // isInMiniApp and status unused in test mode
  const [gameState, setGameState] = useState<GameState | null>(null)

  // DISABLED FOR TESTING: Skip all authentication checks
  // if (status === 'loading') {
  //   return (
  //     <div style={{
  //       display: 'flex',
    ...
    <div style={{
      ...
      <div style={{
        ...
        {user ? (
          ...
          <p style={{ margin: '0.5rem 0 0 0', fontSize: '0.75rem', color: '#999' }}>
            Running in test mode (no authentication)
          </p>
        )}
      </div>
